...
import { Palette, Moon, Sun, Github, Laptop, Cloud, CircleOff } from "lucide-react";
import useMounted from "@/hooks/useMounted";
import { CodeEditorState } from "@/types";
import { setTheme } from "@/store/slices/codeEditorSlice";
...
function ThemeSelector() {
  ...
  const handleThemeSelect = (themeId: string) => {
    dispatch(setTheme(themeId)); // Dispatch the action to update the theme
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <motion.button
        ...
        className="group relative flex items-center gap-2 px-4 py-2.5 sm:px-4 sm:py-2 bg-[#1e1e2e]/80 hover:bg-[#262637] 
      ...
      >
        ...
        <span className="text-gray-300 text-left group-hover:text-white transition-colors flex-1 truncate">
          {currentTheme?.label}
        </span>
        <div
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute top-full left-0 mt-2 w-full sm:w-72 bg-[#1e1e2e]/95 backdrop-blur-xl rounded-xl border border-[#313244] shadow-2xl py-2 z-50"
          >
            ...
            {THEMES.map((t, index) => (
              <motion.button
                ...
                onClick={() => handleThemeSelect(t.id)} // Close the dropdown after selecting
              >
