...
import { removeSnippet } from "@/store/slices/snippetSlice";
import { useDispatch } from "react-redux";
import { deleteSnippet } from "@/actions/snippets.action"; // Import your action function
...
function SnippetCard({ snippet }: { snippet: Snippet }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const result = await deleteSnippet(snippet.id); // Call the action function directly

      if (result?.error) {
        toast.error(result.error); // Handle errors
      } else {
        toast.success("Snippet deleted successfully");
        dispatch(removeSnippet(snippet.id)); // Dispatch to redux to update the UI
      }
    } catch (error) {
